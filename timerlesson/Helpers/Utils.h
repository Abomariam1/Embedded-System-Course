...
#define HIGHER_NIBBLE_CLEAR_REG(REG)					REG=(REG&0X0F)
#define LOWCMDREG(REG,VAL)								REG=(REG & 0xF0)|(VAL&0x0F)
//#define HEIGHCMDREG(REG,VAL)							REG=(REG & 0xF0)|(VAL>>4)  (REG=(REG&0x0F)|(VAL&0XF0))		port = port |(data&0xf0);
#define LNDATA_HNREG(REG, DATA)							(REG=REG|(DATA<<4))
#define HNDATA_HNREG(REG, DATA)							(REG=REG|(DATA&0XF0))
